...
if __name__ == '__main__':
    ...
    load_xfer_no_opt_path=r"C:\Users\pritcham\Documents\mm-framework\multimodal-framework\lit_data_expts\jeong\results\RQ3\D2_Warmstart_NoOpt_final_resMinimal - Copy.csv"
    load_xfer_no_opt_path=r"C:\Users\pritcham\Documents\mm-framework\multimodal-framework\lit_data_expts\jeong\results\RQ3\D2a_WarmNoOpt_FixRF_final_resMinimal - Copy.csv"
    ...
    load_res_path=r"C:\Users\pritcham\Documents\mm-framework\multimodal-framework\lit_data_expts\jeong\results\RQ3\E2_Warmstart_fromGen_samesize_final_resMinimal.csv"
    
    load_RF_path=r"C:\Users\pritcham\Documents\mm-framework\multimodal-framework\lit_data_expts\jeong\results\RQ3\E2a_WarmFromGen_RF10more_final_resMinimal.csv"
    
    load_RF_bad_path=r"C:\Users\pritcham\Documents\mm-framework\multimodal-framework\lit_data_expts\jeong\results\RQ3\E2_Warmstart_fromGen_samesize_final_resMinimal.csv"
    #load_RF_bad_path=r"C:\Users\pritcham\Documents\mm-framework\multimodal-framework\lit_data_expts\jeong\results\RQ3\E2a_WarmFromGen_RF10more_final_resMinimal.csv"
    load_RF_fixed_path=r"C:\Users\pritcham\Documents\mm-framework\multimodal-framework\lit_data_expts\jeong\results\RQ3\E2a_WarmFromGen_FixRF_final_resMinimal.csv"
    ...
    systemUnderTest = 'E2a_WarmFromGen_FixRF'
    
    testset_size = 0.33
        
    if systemUnderTest == 'E2a_WarmFromGen_FixRF':
    ...
    if run_test:
        ...
        for idx,emg_mask in enumerate(emg_masks):
                ...
                emg_others=emg_others[emg_others['ID_run']!=3.0]
                eeg_others=eeg_others[eeg_others['ID_run']!=3.0]
                ...
                for repeat in range(n_repeats):
                   ...
                   # eeg_session2=eeg_ppt[eeg_ppt['ID_run']==2.0]
                    ...
                    augment_scale=(600*4*(2/3))/len(emg_others)
                    # this needs to reduce to FOUR hunded 400 total as thats how many SESH 1 + 2 we have from Subj
                    # if we give it 600, then its got more data to get cold started with, which is unfair
                    emg_train,eeg_train = scale_nonSubj(emg_others,eeg_others,augment_scale)
    ...
    if plot_results:
        ...
        scores_xfer_no_opt=pd.read_csv(load_xfer_no_opt_path,index_col=0)
        scores_RF10_opt=pd.read_csv(load_RF_path,index_col=0)
    ...
    #chance we confuse it eg that by adding non subject within opt, all its doing is learning to firstly
        ...
        scores_xfer_no_opt['calib_level_wholegests']=scores_xfer_no_opt['calib_level']*(1-testset_size)*nGest*nRepsPerGest
        scores_RF10_opt['calib_level_wholegests']=scores_RF10_opt['calib_level']*(1-testset_size)*nGest*nRepsPerGest
 ...
 #       aug_unadjust_agg.plot(y='mean',x='calib_level_wholegests',kind='line',marker='.',ax=ax,rot=0,label='Session 3 augmented\nadjusted split')
        ...
        fig,ax=plt.subplots()
        scores_xfer_no_opt_agg=scores_xfer_no_opt.groupby(['calib_level_wholegests'])['fusion_acc'].agg(['mean','std']).reset_index()
        scores_xfer_no_opt_agg=scores_xfer_no_opt_agg.round({'calib_level_wholegests':5})
        
        scores_agg.plot(y='mean',x='calib_level_wholegests',kind='line',marker='.',ax=ax,rot=0,label='Session 3 transfer\nfrom Generalist')#,yerr='std',capsize=5)
        scores_xfer_no_opt_agg.plot(y='mean',x='calib_level_wholegests',kind='line',marker='.',ax=ax,rot=0,label='Session 3 transfer\nfrom 1 + 2')#,yerr='std',capsize=5)
        ...
        ax.set_ylim(np.floor(scores_minimal['fusion_acc'].min()/0.05)*0.05,np.ceil(scores_minimal['fusion_acc'].max()/0.05)*0.05)
        plt.title('Mean accuracies over subjects on reserved 33% of session 3 (66 gestures)\nwith optimisation not inclusive of Session 3 transfer',loc='left')
        ax.set_xlabel('# Session 3 gestures calibrating (max 134)')
        ax.set_ylabel('Classification Accuracy')#' on reserved 33% (200) subject')
        
        plt.axhline(y=0.86907,label='RQ2 Bespoke\n(Not session-split)',linestyle='--',color='pink')
        plt.axhline(y=0.723,label='RQ1 Generalist\n(Not session-split)',linestyle='--',color='gray')
        #ax.legend(title='Subject',loc='center left',bbox_to_anchor=(1,0.5),ncol=2)
        ax.set_ylim(0.5,1)
        ax.set_ylim(0.6,0.9)
        plt.axhline(y=0.7475,label='Train 1&2 (on\nwider search space)',linestyle='--',color='black')
        ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
        plt.show()
        ...
        if 0:
            fig,ax=plt.subplots()
            scores_RF10_opt_agg=scores_RF10_opt.groupby(['calib_level_wholegests'])['fusion_acc'].agg(['mean','std']).reset_index()
            scores_RF10_opt_agg=scores_RF10_opt_agg.round({'calib_level_wholegests':5})
            
            scores_agg.plot(y='mean',x='calib_level_wholegests',kind='line',marker='.',ax=ax,rot=0,label='Session 3 transfer\nfrom Generalist')#,yerr='std',capsize=5)
            scores_RF10_opt_agg.plot(y='mean',x='calib_level_wholegests',kind='line',marker='.',ax=ax,rot=0,label='Session 3 transfer\nfrom Generalist\nfixed RF xfer')#,yerr='std',capsize=5)
